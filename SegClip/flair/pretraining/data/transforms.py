...
augmentations_pretraining_mask = torch.nn.Sequential(RandomHorizontalFlip(p=0.5),
                                                RandomAffine(p=0.25, degrees=(-5, 5), scale=(0.9, 1)))
...
class LoadMask():
    def __init__(self, target="mask_path"):
        self.target = target

    def __call__(self, data):
       # print(f"Loading mask from: {data[self.target]}") 
        mask = np.array(Image.open(data[self.target]).convert("L"), dtype=np.int32)

        # Add channel dimension: (1, H, W)
        mask = np.expand_dims(mask, 0)

        data[self.target.replace("_path", "")] = mask.astype(np.float32)
        return data
...
class LoadImage():
    ...
    def __call__(self, data):
        #print(f"Loading img from: {data[self.target]}") 
...
class SelectRelevantKeys():
    ...
    def __init__(self, target_keys=None):
        if target_keys is None:
            target_keys = ['image','mask' ,'report', 'sel_category']
